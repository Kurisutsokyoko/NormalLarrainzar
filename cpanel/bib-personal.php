...
<head>
    ...
    <link rel="stylesheet" href="../css/bib-personal.css">
    ...
    <main class="main">
        ...
        <section class="contenido">
            ...
            <section class="container">
                ...
                <div class="all-cards">
                    <?php 
                        $querys = "SELECT * FROM stand ORDER BY levels";
                        $rest = mysqli_query($conn,$querys);
                        while($see1=mysqli_fetch_array($rest)){ 
                    ?>
                    <h2 class="title-doc"><?php echo $see1['stand'] ?></h2>
                    <div class="card">
                        <?php
                            $query="SELECT * FROM personal WHERE orden= $see1[levels]";
                            $resultado=mysqli_query($conn,$query);
                            while($see=mysqli_fetch_array($resultado)){
                        ?>
                            <div class="master">
                                <a class="master-link">
                                    <img src="imagenes/<?php echo $see['imagen'] ?>" class="master-img">
                                    <div class="master-info">
                                        <p class="master-phone"><span class="negritas">Nombre del <?php echo $see['cargo'] ?>: </span><br> <?php echo $see['nombre'] ?> </p>
                                        <p class="master-phone"><span class="negritas">Formación Academica:</span> <?php echo $see['academic_formation'] ?> </p>
                                        <?php if($see['cargo']="Peronal de Apoyo"){ ?>
                                            <p class="master-phone"><span class="negritas">Grado Académicos: </span><br> <?php echo $see['academic_levels'] ?> </p>
                                        <?php } ?>
                                    </div>
                                    <div class="buttons">
                                        <a href="edit-personal.php?id=<?php echo $see['id']; ?>" class="button">edit</a>
                                        <a href="delete-personal.php?id=<?php echo $see['id']; ?>" class="button" name="delete">delete</a>
                                    </div>
                                </a>
                            </div>
                        <?php
