<?php 
  ...
  $id = $_GET['id'];
  $data = "SELECT * FROM personal WHERE id='$id'";
  $resultado=mysqli_query($conn,$data);
...
<head>
    ...
    <link rel="stylesheet" href="../css/edit-info-relev.css">
...
<body>
    ...
    <main class="main">
        ...
        <section class="contenido">
            ...
            <div class="container">
                <h1 class="title-container">Personal</h1>
                <div class="menu-content">
                    <a href="upload-personal.php" class="menu-content-a">Nuevo Elemento</a>
                    <a href="upload-stand.php" class="menu-content-a">Nuevo Puesto</a>
                    <a href="bib-personal.php" class="menu-content-a">Biblioteca</a>
                </div>
                <form action="sub-edit.php" method="post" class="formulario" enctype="multipart/form-data">
                    <div class="content-form">
                        <div class="form-group">
                            <label class="label"  for="my-input">Nombre y Apellido</label><br><br>
                            <textarea id="" class="last" name="nombre" value=""><?php echo $res['nombre']; ?></textarea>
                        </div><br><br>
                        <div class="form-group">
                            <label class="label"  for="my-input">Fotografia</label><br><br>
                            <input id="my-input" class="imagen" type="file" name="imagen" value="">
                            <label class="label">Anterior Portada</label>
                            <img src="imagenes/<?php echo $res['imagen']; ?>" alt="" class="muestra-photo">
                        </div>
                        <input type="hidden" id="id" name="id" value="<?php echo $res['id']; ?>">
                        <input type="submit" value="Guardar" class="boton" name="Guardar-Personal">
                    </div>
                    <div class="content-form"><!--
                        <div class="content-form">
                            <label class="label"  for="my-input">Grado Académico</label>
                            <br>
                            <br>
                            <input name="academic_level" class="name-lastname" value="<?php echo $res['academic_levels']; ?>">
                        </div>-->
                        <div class="form-grou2">
                            <label for="my-input" class="label">Área de Trabajo</label><br><br>
                            <!--<input type="text" name="funcion" class="name-lastname" placeholder="Docente/Administrativo/...">-->
                            <select name="cargo" onchange="visible(this.value)">
                                <option value="<?php echo $res['cargo'] ?>,<?php echo $res['orden']; ?>" selected><?php echo $res['orden']; echo ' - '; echo $res['cargo'] ?></option>
                                <?php
                                    $query="SELECT * from stand ORDER BY levels";
                                    $resultado=mysqli_query($conn,$query);
                                    while($see=mysqli_fetch_array($resultado)){
                                ?>
                                    <option value="<?php echo $see['stand'] ?>,<?php echo $see['levels'] ?>"><?php echo $see['levels']; echo ' - '; echo $see['stand'] ?></option>
                                <?php
                                    }
                                ?>
                            </select>
                        </div>
                        <div class="" id="ckeditorr" style="visibility:hidden;">
                            <label for="my-input" class="label">Formación Academica</label>
                            <br>
                            <br>
                            <textarea name="academic_formation" id="ckeitor" class="ckeitor" cols="120" rows="20" ><?php echo $res['academic_formation']; ?></textarea>
                        </div>
                    </div>
                </form>
            </div>
    ...
    {
        let valors = valor;
        const constante = valors.split(",",1);
        if (constante == "Personal de Apoyo"){
            document.getElementById("ckeditorr").style.visibility = "hidden"
        }else if(valor==0){
            document.getElementById("ckeditorr").style.visibility = "hidden"
        }else{
            document.getElementById("ckeditorr").style.visibility = "visible"
            document.getElementById("ckeditorr").value = valor ;
        }
    }
</script>
</html>