...
<body>
    ...
    <main class="main">
        ...
        <section class="container">
            <h1 class="title-container">Información Relevante</h1>
            <div class="container-content">
                <?php
                    $query="SELECT * from informacion ORDER BY id DESC";
                    $resultado=mysqli_query($conn,$query);
                    while($see=mysqli_fetch_array($resultado)){
                ?>
               <a href="info.php?id=<?php echo $see['id']; ?>" class="card">
                    <img src="cpanel/imagenes/<?php echo $see['imagen'] ?>" alt="" class="card-img">
                    <div class="content-card">
                        <h3 class="title-card"><?php echo $see['titulo'] ?></h3>
                        <p class="content-text" ><?php echo $see['fecha'] ?></p>
                        <input type="hidden" id="id" name="id" value="<?php echo $see['id']; ?>">
                    </div>
                </a>
               <?php 
                    }
